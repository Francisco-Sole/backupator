		...
		function actualizaDatos(){
			...
			if (parseInt(numTablaPequenas) == parseInt(oks) && !primeraVez) {
				...
				timer3 = setInterval(function(){ 
					//var datos = {"tabla": nombre, "count":count, "current": current, "variable": variable, "indice": indice};
					indice = listaIdTablasGrandes[index];
					nombre = tamanos[indice].nombre;
					count = tamanos[indice].count;
					tram = tamanos[indice].tramos;
					...
					$.ajax({
						...
						success: function (data) {
							...
							//tamanos[data.indice].realizado[parseInt(data.variable)] = 1;
							// globalPequenas++; 
							if (parseInt(data.variable)-10 < 0) {
								data.variable = 0;
							}else{
								data.variable -= 10;
							}
							...
							//pintamos barras como OK
							$("#contador_"+data.nombre).html(parseInt(data.current));
							$("#carga_"+data.nombre).css("width", ((parseInt(data.current))/parseInt(data.count))*100+"%");
							$("#carga_"+data.nombre).css("background-color", "KHAKI");
							$("#carga_"+data.nombre).attr("cargado", ((parseInt(data.current))/parseInt(data.count))*100);
							...
							// if (tamanos[data.indice].realizado[parseInt(data.variable)+99] == 0) {
							// 	do_backupB(tamanos[data.indice].nombre, tamanos[data.indice].count, data.current , parseInt(data.variable)+1, data.indice);
							// }else{
							// 	$("#contador_"+data.tabla).html(data.count);
							// 	$("#carga_"+data.tabla).css("width", "100%");
							// 	$("#carga_"+data.tabla).attr("cargado", "100");
					...
					//pintamos barras como pedidos
					$("#contador_"+nombre).html(parseInt(current));
					$("#carga_"+nombre).css("width", ((parseInt(current))/parseInt(count))*100+"%");
					$("#carga_"+nombre).css("background-color", "silver");
					$("#carga_"+nombre).attr("cargado", ((parseInt(current))/parseInt(count))*100);
					...
					// if (tamanos[indice].realizado[parseInt(variable)+99] == 0) {
					// 	inmediatewindow = window.open("backupB.php?tabla="+ nombre + "&count="+ count + "&current=" + current + "&variable=" + variable + "&indice=" + indice );
					// 	// for (var i = 0; i < 100; i++) {
					// 	// 	tamanos[indice].realizado[parseInt(variable)+i] = 1;
					// 	// 	oksreg++;
					// 	// 	globalGrandes++; 
					// 	// }
					// 	variable += 100;
					...
					// }else{
					// 	//falta codigo...
					// 	console.log("quedan menos de 100 para teminar", nombre);
					// 	do_backup(nombre, count, current , parseInt(variable)+1, indice);
					// 	index++;
					// 	variable  = 0;
					// 	current = 0;
					// }

					if (index == listaIdTablasGrandes.length-1) {
